(() => {
    ...
    function initSliders() {
        bildSliders();
        if (document.querySelector(".hero-main__slider")) new core(".hero-main__slider", {
        ...
        if (document.querySelector(".brands-main__slider")) new core(".brands-main__slider", {
    ...
    const renderCartOrder = () => {
        ...
        if (JSON.parse(localStorage.getItem("products"))) {
            JSON.parse(localStorage.getItem("products")).forEach((cartProduct => {
                totalPrice += Number(cartProduct.price) * Number(cartProduct.quantity);
            }));
    ...
    function addToCartWithFfyEffect(productButton, productId) {
    ...
    function addToCart(productButton, productId) {
        ...
        if (JSON.parse(localStorage.getItem("products"))) {
            ...
            if (-1 !== existProductIndex) cartProductsList[existProductIndex].quantity = cartProductsList[existProductIndex].quantity + 1; else cartProductsList.push({
                id: productId,
                imageUrl: cartProductImageUrl,
                title: cartProductTitle,
                price: cartProductPrice.split(" ")[0].split("&nbsp;").join(""),
                quantity: 1
            });
        } else cartProductsList.push({
            id: productId,
            imageUrl: cartProductImageUrl,
            title: cartProductTitle,
            price: cartProductPrice.split(" ")[0].split("&nbsp;").join(""),
        ...
        renderCartBody();
        renderCartOrder();
        if (localStorage.getItem("products")) cartIcon.innerHTML = `<span>${JSON.parse(localStorage.getItem("products")).length}</span>`; else cartIcon.insertAdjacentHTML("beforeend", `<span>${JSON.parse(localStorage.getItem("products")).length}</span>`);
    ...
    function deleteFromCart(productId) {
        if (window.confirm("Вы действительно хотите удалить товар?")) {
            ...
            renderCartBody();
            renderCartOrder();
            if (JSON.parse(localStorage.getItem("products")) && JSON.parse(localStorage.getItem("products")).length > 0) cartIcon.innerHTML = `<span>${JSON.parse(localStorage.getItem("products")).length}</span>`; else cartIcon.innerHTML = "";
        }
    }
    function minusProduct(productId) {
        if (JSON.parse(localStorage.getItem("products"))) {
            const existProductIndex = cartProductsList.findIndex((cartProduct => cartProduct.id === productId));
            if (-1 !== existProductIndex) cartProductsList[existProductIndex].quantity = cartProductsList[existProductIndex].quantity - 1;
        }
        cartList.innerHTML = "";
        localStorage.setItem("products", JSON.stringify(cartProductsList));
        renderCartBody();
    ...
    const lovely_status = document.querySelector(".status-header");
    const statusIcon = lovely_status.querySelector(".status-header__icon");
    if (JSON.parse(localStorage.getItem("lovely"))) lovelyProductsList = JSON.parse(localStorage.getItem("lovely"));
    ...
    function addToLovely(targetElement, productId) {
        const product = document.querySelector(`[data-pid="${productId}"]`);
        const cartProductImageUrl = product.querySelector(".item-new-products__image").src;
        const cartProductTitle = product.querySelector(".item-new-products__title").innerHTML;
        const cartProductPrice = product.querySelector(".item-new-products__price").innerHTML;
        if (JSON.parse(localStorage.getItem("lovely"))) {
            ...
            if (lovelyProductExist) return;
        }
        lovelyProductsList.push({
            id: productId,
            imageUrl: cartProductImageUrl,
            title: cartProductTitle,
            price: cartProductPrice.split(" ")[0].split("&nbsp;").join(""),
        ...
        renderLovelyBody();
        if (localStorage.getItem("lovely")) statusIcon.innerHTML = `<span>${JSON.parse(localStorage.getItem("lovely")).length}</span>`; else statusIcon.insertAdjacentHTML("beforeend", `<span>${JSON.parse(localStorage.getItem("lovely")).length}</span>`);
    }
    function deleteFromLovely(productButton, productId) {
        if (window.confirm("Удалить товар из избранного?")) {
            ...
            lovelyProductsList = [];
            renderLovelyBody();
            if (JSON.parse(localStorage.getItem("lovely")) && JSON.parse(localStorage.getItem("lovely")).length > 0) statusIcon.innerHTML = `<span>${JSON.parse(localStorage.getItem("lovely")).length}</span>`; else statusIcon.innerHTML = "";
        }
    }
    function clearLovely() {
    ...
    function documentActions(e) {
        ...
        if (targetElement.classList.contains("header__contacts-block-title")) document.querySelector(".contacts").classList.add("active"); else if (!targetElement.closest(".contacts") || targetElement.classList.contains("contacts__top") || targetElement.closest(".contacts__top")) document.querySelector(".contacts").classList.remove("active");
        if (targetElement.classList.contains("header__catalog-title")) document.querySelector(".menu-catalog").classList.add("active"); else if (!targetElement.closest(".menu-catalog") || targetElement.closest(".menu-catalog__close")) document.querySelector(".menu-catalog").classList.remove("active");
        ...
        if (targetElement.classList.contains("header__cart") || targetElement.closest(".header__cart")) {
            if (document.querySelector(".cart-list").children.length > 0) document.querySelector(".header__cart").classList.add("_active-cart");
            e.preventDefault();
        }
        if (targetElement.closest(".cart-header__top") || !targetElement.closest(".cart-header__list") && !targetElement.classList.contains("item-new-products__button") && !targetElement.closest(".header__cart")) document.querySelector(".header__cart").classList.remove("_active-cart");
        ...
        if (targetElement.classList.contains("header__burger-icon") || targetElement.closest(".header__burger-icon")) document.querySelector(".header__bottom-content").classList.toggle("active");
        if (targetElement.classList.contains("status__current") || targetElement.closest(".status__current")) targetElement.closest(".status").classList.toggle("active");
        if (targetElement.classList.contains("status-header__icon") || targetElement.closest(".status-header__icon")) {
            if (document.querySelector(".status-list").children.length > 0) document.querySelector(".header__status").classList.add("_active-status");
            e.preventDefault();
        }
        if (targetElement.closest(".status-header__top") || !targetElement.closest(".status-header__list") && !targetElement.classList.contains("status__item_like") && !targetElement.closest(".header__status")) document.querySelector(".header__status").classList.remove("_active-status");
